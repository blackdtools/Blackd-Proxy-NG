...
Begin VB.Form frmLoading 
   ...
   Begin VB.Label Label1 
      BackColor       =   &H00000000&
      Caption         =   "Loading Blackd Proxy NG"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   13.5
         Charset         =   0
...
'  If Ammount < 25 Then
'    lblProgress.ForeColor = &H80000012
...
MustRegister:
    ...
    If (fso.FileExists(strSys) = False) Then
          If MsgBox("Bad installation of " & currentAppName & vbCrLf & "Unable to find " & strSys & vbCrLf & "Do you want to try copying it there?", _
           vbYesNo + vbQuestion, currentAppName & " - Unable to fix Directx 7 support") = vbYes Then
            blnUserAnswer = True
    ...
    If (MsgBox(currentAppName & " requires installing Directx7." & _
    vbCrLf & "If you press the OK button then " & currentAppName & " will try to install it," & vbCrLf & _
    "then it will close. Finally you must manually reload " & currentAppName & "." & vbCrLf & vbCrLf & _
    currentAppName & " will attempt to execute this:" & vbCrLf & _
    strAll, vbOKCancel + vbQuestion, currentAppName & " - Action required") = vbOK) Then
      Shell strAll
      End
    Else
      End
...
giveError:
  ...
  If Err.Number = 339 Then
    ...
    vbOKOnly + vbCritical, currentAppName & " " & ProxyVersion & " - Critical error"
    startError = "Dim Y"
    Dim Y
  ...
  Else
    ...
    " - " & currentAppName & " not installed correctly" & vbCrLf & _
    " Details:" & vbCrLf & _
...
'  Dim correctName As String
  ...
  If App.PrevInstance Then
    If MsgBox(currentAppName & " is already open. You should not open it twice." & vbCrLf & _
  ...
  If IsInIDE() = True Then
    MsgBox currentAppName & " won't run properly from VB6 IDE in final mode (because a problem with crackd.dll path)" & _
     vbCrLf & "For debug you should replace all:" & vbCrLf & "#Const FinalMode =0" & vbCrLf & "to" & vbCrLf & "#Const FinalMode =0" & vbCrLf & vbCrLf & "You should also ensure that crackd.dll is found in C:\BlackdProxyNG\Blackd-Proxy-NG\", vbOKOnly + vbInformation, "Information for the programmer"
    End
  End If
  #End If
