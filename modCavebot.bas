  ...
  If res = True Then
     ...
     Else
        If IsNumeric(actionLine) = False Then
            aRes = GiveGMmessage(Sid, "Cavebot syntax error: expecting to jump to a line number. You typed something can NOT be converted to a number: " & actionLine, currentAppName)
            DoEvents
 ...
 ' SendLogSystemMessageToClient sid, "executing " & CStr(currLineNumber) & " : " & currLine
  ...
  Case "autoexploring"
    ...
    'val4 = Int(Rnd * 20 + 1)
            ...
            'PerformMove Sid, (val1 + val4), (val2 + val4), val3
            DoRandomMove Sid
            'exeLine(Sid) = exeLine(Sid) + 1
           'DoEvents
    ...
    Else ' doing exorivis
        ...
        If (xt = X + 1) And (yt = Y) And (mydir = 1) Then
            If Round((myHP(idConnection) / myMaxHP(idConnection)) * 100) >= frmCavebot.scrollExorivis.Value Then
                ...
                Else
                    If PlayTheDangerSound = False Then
                        aRes = GiveGMmessage(idConnection, "Low of mana: can't use " & spellToUse, currentAppName)
                        DoEvents
...
Public Sub AddCavebotMoveN()
'aqui teste emplacemente N

  If cavebotIDselected > 0 Then
    frmCavebot.AddScriptLine "move " & myX(cavebotIDselected) & "," & myY(cavebotIDselected) + 1 & "," & myZ(cavebotIDselected)
  End If
End Sub
Public Sub AddCavebotMoveAE()
'aqui teste eauto explo

  If cavebotIDselected > 0 Then
  'frmCavebot.AddScriptLine "autoexp"
    frmCavebot.AddScriptLine "autoexploring"
    'frmCavebot.AddScriptLine "autoexp " & myX(cavebotIDselected) & "," & myY(cavebotIDselected) + 3 & "," & myZ(cavebotIDselected)
  End If
End Sub
