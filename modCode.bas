...
Option Explicit
Public Const currentAppName As String = "Blackd Proxy NG"
...
"config1033,config1034,config1035,config1036,config1037,config1038,config1039,config1040,config1041,config1050,config1051,config1051preview,config1052,config1052preview,config1053,config1053preview,config1054,config1055,config1056,config1057,config1058,config1059,config1060,config1061,config1062,config1063,config1064,config1070,config1071,config1072,config1073,config1074,config1075,config1076,config1077,config1078,config1079,config1080,config1081,config1082"

Public Const FIX_addConfigVersions As String = _
"10.33,10.34,10.35,10.36,10.37,10.38,10.39,10.4,10.41,10.5,10.51,10.51 preview,10.52,10.52 preview,10.53,10.53 preview,10.54,10.55,10.56,10.57,10.58,10.59,10.60,10.61,10.62,10.63,10.64,10.70,10.71,10.72,10.73,10.74,10.75,10.76,10.77,10.78,10.79,10.80,10.81,10.82"

Public Const FIX_addConfigVersionsLongs As String = _
"1033,1034,1035,1036,1037,1038,1039,1040,1041,1050,1051,1051,1052,1052,1053,1053,1054,1055,1056,1057,1058,1059,1060,1061,1062,1063,1064,1070,1071,1072,1073,1074,1075,1076,1077,1078,1079,1080,1081,1082"

Public Const FIX_highestTibiaVersionLong As String = "1082"
Public Const FIX_TibiaVersionDefaultString As String = "10.82"
Public Const FIX_TibiaVersionForceString As String = "10.82"
...
Public Sub ModifyTibiaIPs2()
  ...
  If ReadyToChangeTibiaIP() = False Then
    Debug.Print "Warning: " & currentAppName & " was not ready to modify Tibia Ips yet"
    Exit Sub
  End If
  'hWndDesktop = GetDesktopWindow()
...
lastcheck:
    If (frmHardcoreCheats.chkRuneAlarm.Value = 1) And (CInt(frmHardcoreCheats.txtAlarmUHs.Text) > fRes.foundcount) Then
      If PlayTheDangerSound = False Then
        'ChangePlayTheDangerSound True
        If fRes.foundcount = 0 Then
            aRes = GiveGMmessage(idConnection, "Can't find UH, open bp", "Warning")
        Else
            aRes = GiveGMmessage(idConnection, "Low of UH", "Warning")
        End If
        DoEvents
        UseUH = -1
        Exit Function
...
lastcheck:
    If (frmHardcoreCheats.chkRuneAlarm.Value = 1) And (CInt(frmHardcoreCheats.txtAlarmUHs.Text) > fRes.foundcount) Then
      If PlayTheDangerSound = False Then
        'ChangePlayTheDangerSound True
        If fRes.foundcount = 0 Then
            aRes = GiveGMmessage(idConnection, "Can't find UH, open new bp", currentAppName)
        Else
            aRes = GiveGMmessage(idConnection, "Low of UH", currentAppName)
        End If
        DoEvents
        UseFastUH = -1
        Exit Function
...
Public Function EvalClientMessage(ByVal idConnection As Integer, ByRef packet() As Byte, pos As Long) As Long
  ...
  If (lmsg = "antipush") Then
      If RuneMakerOptions(idConnection).autoAp = False Then
      RuneMakerOptions(idConnection).autoAp = True
      ...
      Else
      RuneMakerOptions(idConnection).autoAp = False
  ...
  'aqui auto ssa
    If (lmsg = "autossa") Then
      If RuneMakerOptions(idConnection).autossa = False Then
      RuneMakerOptions(idConnection).autossa = True
      aRes = ExecuteInTibia("exiva < B4 15 $hex-tibiastr:Auto SSA ON$", idConnection, True)
      aRes = SendCustomSystemMessageToClient(idConnection, "Auto SSA ON", &HB)
      Else
      RuneMakerOptions(idConnection).autossa = False
      aRes = ExecuteInTibia("exiva < B4 15 $hex-tibiastr:Auto SSA OF$", idConnection, True)
      aRes = SendCustomSystemMessageToClient(idConnection, "Auto SSA OFF", &HB)
      End If
  EvalClientMessage = 1
  Exit Function
  End If
  ...
  'aqui pushmax
    If (lmsg = "pushmax") Then
      If RuneMakerOptions(idConnection).autopmax = False Then
      RuneMakerOptions(idConnection).autopmax = True
      ...
      Else
      RuneMakerOptions(idConnection).autopmax = False
  ...
  End If
    ...
    If (lmsg = "collectitems") Then
      'If TrainerOptions(idConnection).enabled = 0 Then
      'TrainerOptions(idConnection).enabled = 1
      'TrainerOptions(idConnection).enabled = True
      'aRes = SendCustomSystemMessageToClient(idConnection, "Collectitems ON", &HB)
      aRes = ExecuteInTibia("Exiva > 78 $numbertohex2:{$myx$}$ $numbertohex2:{$myy$}$ $numbertohex1:{$myz$}$ D7 0B 01 FF FF 40 00 01 01", idConnection, True)
      aRes = ExecuteInTibia("Exiva > 78 $numbertohex2:{$numericalexp:{$myx$}+1$}$ $numbertohex2:{$myy$}$ $numbertohex1:{$myz$}$ D7 0B 01 FF FF 40 00 01 01", idConnection, True)
      aRes = ExecuteInTibia("Exiva > 78 $numbertohex2:{$numericalexp:{$myx$}-1$}$ $numbertohex2:{$myy$}$ $numbertohex1:{$myz$}$ D7 0B 01 FF FF 40 00 01 01", idConnection, True)
      aRes = ExecuteInTibia("Exiva > 78 $numbertohex2:{$myx$}$ $numbertohex2:{$numericalexp:{$myy$}+1$}$ $numbertohex1:{$myz$}$ D7 0B 01 FF FF 40 00 01 01", idConnection, True)
      aRes = ExecuteInTibia("Exiva > 78 $numbertohex2:{$myx$}$ $numbertohex2:{$numericalexp:{$myy$}-1$}$ $numbertohex1:{$myz$}$ D7 0B 01 FF FF 40 00 01 01", idConnection, True)
      aRes = ExecuteInTibia("Exiva > 78 $numbertohex2:{$numericalexp:{$myx$}+1$}$ $numbertohex2:{$numericalexp:{$myy$}+1$}$ $numbertohex1:{$myz$}$ D7 0B 01 FF FF 40 00 01 01", idConnection, True)
      aRes = ExecuteInTibia("Exiva > 78 $numbertohex2:{$numericalexp:{$myx$}-1$}$ $numbertohex2:{$numericalexp:{$myy$}+1$}$ $numbertohex1:{$myz$}$ D7 0B 01 FF FF 40 00 01 01", idConnection, True)
      aRes = ExecuteInTibia("Exiva > 78 $numbertohex2:{$numericalexp:{$myx$}+1$}$ $numbertohex2:{$numericalexp:{$myy$}-1$}$ $numbertohex1:{$myz$}$ D7 0B 01 FF FF 40 00 01 01", idConnection, True)
      aRes = ExecuteInTibia("Exiva > 78 $numbertohex2:{$numericalexp:{$myx$}-1$}$ $numbertohex2:{$numericalexp:{$myy$}-1$}$ $numbertohex1:{$myz$}$ D7 0B 01 FF FF 40 00 01 01", idConnection, True)
      'Else
      'TrainerOptions(idConnection).enabled = 0
      'TrainerOptions(idConnection).enabled = False
      'aRes = SendCustomSystemMessageToClient(idConnection, "Collectitems OFF", &HB)
      'End If
  EvalClientMessage = 1
  Exit Function
  End If
  
    'aqui tentativa target
      If (lmsg = "holdtarget") Then
        If RuneMakerOptions(idConnection).autotar = False Then
  ...
  End If
        ...
        If (lmsg = "holdtarget2") Then
        'esse vai executar a ordem de kill process 2
  ...
  'aqui tentativa dash
  'If (lMsg = "dash") Then
  '  If frmRunemaker.chkDash.Value = 0 Then
  '      frmRunemaker.chkDash.Value = 1
  '  End If
  '  EvalClientMessage = 1
  '  Exit Function
  '  End If
  ...
  End If
      ...
      If (lmsg = "sdmax1") Then
    aRes = ExecuteInTibia("exiva 5", idConnection, True)
  DoEvents
  EvalClientMessage = 1
  Exit Function
  End If
        ...
        If (lmsg = "sdmax") Then
      If RuneMakerOptions(idConnection).autoSdt = False Then
        RuneMakerOptions(idConnection).autoSdt = True
        aRes = SendCustomSystemMessageToClient(idConnection, "SDMAX ON", &HB)
      Else
        RuneMakerOptions(idConnection).autoSdt = False
      ...
      Case Else
       ...
       ElseIf rightpart = "c" Then ' cast icicle
        ...
        ElseIf rightpart = "version" Then
          res = 1
          'ProxyVersion
          aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " version: " & ProxyVersion)
          DoEvents
...
Public Function ExecuteInTibia(spellString As String, idConnection As Integer, cantBeIgnored As Boolean, Optional onlyCommands As Boolean = False) As Integer
  ...
  If (sok = True) And i > 2 Then
    ...
    ElseIf aRes = 0 Then
      If ((GetTickCount() > nextAllowedmsg(idConnection)) Or (cantBeIgnored = True)) Then
        ...
        End If
        DoEvents
        aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " sent msg to " & msgto & " : " & theRealMsg)
        DoEvents
      End If
      ExecuteInTibia = 0
    Else
  ...
  ElseIf aRes = 0 Then
    If ((GetTickCount() > nextAllowedmsg(idConnection)) Or (cantBeIgnored = True)) Then
      nextAllowedmsg(idConnection) = GetTickCount() + DELAYBETWEENAUTOMSG_ms
      If (GameConnected(idConnection) = True) Then
        If (frmMain.sckServerGame(idConnection).State = sckConnected) Then
            frmMain.UnifiedSendToServerGame idConnection, cPacket, True
        End If
      Else
        ExecuteInTibia = -1
...
Public Function SellInTrade(ByRef idConnection As Integer, ByVal strMsg As String) As Long
   ...
   ' only for tibia 8.2 +
    ...
    If (doingTrade2(idConnection) = False) Then
        aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to sell that: trade window is not opened")
        DoEvents
        SellInTrade = 0
        Exit Function
    End If
    If Len(strMsg) < 7 Then
        aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to sell that: bad format in parameters (too short)")
        DoEvents
        SellInTrade = 0
        Exit Function
    End If
    If Mid(strMsg, 6, 1) <> ":" Then
        aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to sell that: bad format in parameters (missing separator)")
        DoEvents
        SellInTrade = 0
        Exit Function
    End If
    ...
    If IsNumeric(strAm) = False Then
        aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to sell that: bad format in parameters (bad amount)")
        DoEvents
        SellInTrade = 0
        Exit Function
    End If
    ...
    If CLng(strAm) <= 0 Then
        aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to sell that: amount should be positive")
        SellInTrade = 0
        Exit Function
    End If
    ...
    Exit Function
goterr:
    aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to sell that: unknown error, vb err code " & CStr(Err.Number))
    DoEvents
    SellInTrade = -1
End Function
...
Public Function BuyInTrade(ByRef idConnection As Integer, ByVal strMsg As String) As Long
   ...
   ' only for tibia 8.2 +
    ...
    #End If
    If (doingTrade2(idConnection) = False) Then
        aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to buy that: trade window is not opened")
        DoEvents
        BuyInTrade = 0
        Exit Function
    End If
    If Len(strMsg) < 7 Then
        aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to buy that: bad format in parameters (too short)")
        DoEvents
        BuyInTrade = 0
        Exit Function
    End If
    If Mid(strMsg, 6, 1) <> ":" Then
        aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to buy that: bad format in parameters (missing separator)")
        DoEvents
        BuyInTrade = 0
        Exit Function
    End If
    strAm = Right$(strMsg, Len(strMsg) - 6)
    If IsNumeric(strAm) = False Then
        aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to buy that: bad format in parameters (bad amount)")
        DoEvents
        BuyInTrade = 0
        Exit Function
    End If
    ...
    If CLng(strAm) <= 0 Then
        aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to buy that: amount should be positive")
        BuyInTrade = 0
        Exit Function
    End If
    ...
    Exit Function
goterr:
    aRes = SendLogSystemMessageToClient(idConnection, currentAppName & " is unable to buy that: unknown error, vb err code " & CStr(Err.Number))
    DoEvents
    BuyInTrade = -1
End Function
...
End Sub
Public Function ProcessKillOrder3(idConnection As Integer) As Long
  Dim aRes As Long
  Dim myS As Byte
  Dim lTarget As String
  Dim sCheat As String
  ...
  On Error GoTo goterr
  #End If
 currTargetName(idConnection) = "test"
 target = currTargetName(idConnection)
  'If (target = "") Or (target = " ") Then ' use last targeted
  ''aqui teste target
  '  target = currTargetName(idConnection)
...
'target = nameofgivenID
  If (GotKillOrderTargetID(idConnection) = 0) Then
    RuneMakerOptions(idConnection).autoarme4 = False
    RuneMakerOptions(idConnection).autoarme5 = False
    GotKillOrder(idConnection) = False
    'aRes = GiveGMmessage(idConnection, "- " & target & " -", "Sorry, target NOT FOUND")
    DoEvents
  Else
  RuneMakerOptions(idConnection).autoarme5 = True
    'GotKillOrder(idConnection) = True
    'RuneMakerOptions(idConnection).autoarme4 = True
    'GotKillOrderTargetName(idConnection) = target
    'aRes = GiveGMmessage(idConnection, ">> " & GotKillOrderTargetName(idConnection) & " <<", "TESTE")
    'DoEvents
    'ThinkTheKill (idConnection)
    'DoEvents
  End If
