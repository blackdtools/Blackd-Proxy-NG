...
Public Const ProxyVersion = "2.9" ' Proxy version ' string version
Public Const myNumericVersion = 290 ' numeric version
...
Public targetIDselected As Integer
...
Public Function IsGM(str As String) As Boolean
  ...
  If frmAdvanced.chkStopOnGM.Value = 0 Then
     IsGM = False
     Exit Function
  End If
  If str <> "" Then
...
Public Function GetTheMobileInfo(idConnection As Integer, ByRef packet() As Byte, firstPos As Long) As TypePlayerInfo
  ...
  If outfitType = &H0 Then ' thing outfit
    If (packet(resF.pos + 1) = &H0) And (packet(resF.pos + 2) = &H0) Then
      'unhide invis beings
      If (resF.newID <> myID(idConnection)) And (frmRunemaker.chkReveal.Value = 1) Then
    ...
    ElseIf idTile = &H62 Then
      ...
      If outfit = 0 Then
        If (packet(pos + 9) = &H0) And (packet(pos + 10) = &H0) Then
          If (nameofgivenID <> CharacterName(idConnection)) And (frmRunemaker.chkReveal.Value = 1) Then
 ...
 ' myres.gotBlankRune = False
  ...
  ' tibia 10.36:6A C4 81 C3 7E 07 FF 62 00 3A A7 3B 02 62 00 86 00 09 00 72 5D 03 00 00 06 1D EE 00 00 00 00 00 FF 00 00 00
       ...
       ' frmMain.txtPackets.Text = frmMain.txtPackets.Text & vbCrLf & "New char1 (" & nameofgivenID & ") with level " & CStr(templ2)
        ...
        If templ1 = 0 Then
          If (packet(pos + 9) = &H0) And (packet(pos + 10) = &H0) Then
            If (tempID <> myID(idConnection)) And (frmRunemaker.chkReveal.Value = 1) Then
              packet(pos + 9) = LowByteOfLong(tileID_Oracle)
              packet(pos + 10) = HighByteOfLong(tileID_Oracle)
            End If
          End If
   ...
   ' debug872= 6E 02 CA 1F 00 1B 00 74 68 65 20 72 65 6D 61 69 6E 73 20 6F 66 20 61 6E 20 65 6C 65 6D 65 6E 74 61 6C 0E 00 01 D7 0B 25  Last good position=2 x=32398;y=32726;z=3
    ...
    Case &H86
     ...
     ' aRes = SendSystemMessageToClient(idConnection, "You are being attacked by " & nameOfGivenID)
      'DangerPK
      If RuneMakerOptions(idConnection).autora = True Then
      ProcessKillOrder idConnection, nameofgivenID
      End If
    ...
    Case &H8E
      ...
      If (outfitType = 0) Then ' thing outfit
        If (packet(pos + 1) = &H0) And (packet(pos + 2) = &H0) And (tempID <> myID(idConnection)) And (frmRunemaker.chkReveal.Value = 1) Then
          nameofgivenID = GetNameFromID(idConnection, tempID)
    ...
    Case &H9D
      ...
      ' Parser fixed 100% in Blackd Proxy NG 1.3
    ...
    Case &HA3
      ...
      If TibiaVersionLong >= 860 Then
          ...
          Else
           ...
           aRes = GiveGMmessage(idConnection, "RELOG ASAP. Don't activate the cavebot again until you relog. Else you risk for a sure ban. And please send errors.txt to daniel@blackdtools.com", currentAppName)
           CheatsPaused(idConnection) = True
           ChangePlayTheDangerSound True
           DoEvents
          End If
    ...
    Case &HB4
      ...
      If (TibiaVersionLong >= 1055) Then
        ...
        Case &H19, &H1A, &H1C, &H1D
              ' B4 1D A5 7F 91 7B 09 10 00 00 00 D7 29 00 43 79 6B 65 74 20 4B 6E 69 67 68 74 20 67 61 69 6E 65 64 20 31 36 20 65 78 70 65 72 69 65 6E 63 65 20 70 6F 69 6E 74 73 2E
        ...
        Case &H6, &H21, &H22
                   ...
                   ' B4 22 30 4E 56 00 41 6C 67 61 74 61 20 53 74 6F 72 20 68 61 73 20 62 65 65 6E 20 69 6E 76 69 74 65 64 2E 20 4F 70 65 6E 20 74 68 65 20 70 61 72 74 79 20 63 68 61 6E 6E 65 6C 20 74 6F 20 63 6F 6D 6D 75 6E 69 63 61 74 65 20 77 69 74 68 20 79 6F 75 72 20 6D 65 6D 62 65 72 73 2E 91 FA D5 7B 02 04 90 FA D5 7B 02 00 94 FA D5 7B 02 02 00 91 0A F7 DC 02 02 94 0A F7 DC 02 00 00 91 FA D5 7B 02 04 94 FA D5 7B 02 02 00
                    ...
                    Next itemCount
        Case Else
          ' B4 1F 18 00 4C 6F 6F 74 20 6F 66 20 61 20 74 72 6F 6C 6C 3A 20 6E 6F 74 68 69 6E 67
                         'Debug.Print frmMain.showAsStr3(packet, True, pos, 1000000)
                    pos = pos + 2
                    lonN = GetTheLong(packet(pos), packet(pos + 1))
                    pos = pos + 2
  ...
  If GameConnected(idConnection) = True Then
    ...
    If GotPacketWarning(idConnection) = False Then
      ...
      If learnResult.fail = True Then
        ...
        aRes = GiveGMmessage(idConnection, currentAppName & " got critical error! Safe mode ON . Alarm on. Report log details (errors.txt) to daniel@blackdtools.com . Relog to reactivate cheats", "Blackd")
        ChangePlayTheDangerSound True
        DoEvents
        GoTo exitL
      End If
      ...
      If sentFirstPacket(idConnection) = True And sentWelcome(idConnection) = False Then
        ...
        If TrialVersion = True Then
          ...
          ElseIf GotTrialLock = False Then
            aRes = GiveGMmessage(idConnection, "Welcome to your trial of " & currentAppName & " v" & ProxyVersion & " ! Cheats ENABLED. Official site: www.blackdtools.com", "Blackd")
            DoEvents
          Else
        ...
        Else
          If LimitedLeader = "-" Then
            aRes = GiveGMmessage(idConnection, "Welcome to " & currentAppName & " v" & ProxyVersion & " ! Official site: www.blackdtools.com", "Blackd")
            DoEvents
          Else
            aRes = GiveGMmessage(idConnection, "Welcome! This is a limited version of " & currentAppName & " v" & ProxyVersion & " - Cheats are limited to allow sync attack orders from me", LimitedLeader)
            DoEvents
          End If
        End If
        End If
      ...
      If (AfterLoginLogoutReason(idConnection) <> "") Then 'And (CheatsPaused(idConnection) = False) Then
        ...
        aRes = GiveServerError(currentAppName & " logout - logged out because : " & AfterLoginLogoutReason(idConnection), idConnection)
        DoEvents
...
If pos < 2 Then
    MsgBox "Bad use of function RemoveBytesFromPacket. Can't edit header", vbCritical + vbOKOnly, "Internal error of " & currentAppName
    End
End If
If uboundAFTER < 0 Then
    MsgBox "Bad use of function RemoveBytesFromPacket. Resulting packet would be null", vbCritical + vbOKOnly, "Internal error of " & currentAppName
    End
End If
' change header
